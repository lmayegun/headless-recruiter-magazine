...
function FuseSearch(props)
{
    ...
    const searchText = useSelector(({search}) => search.searchText.searchTextState);
    ...
    // useEffect(() => {
    //     // reduxDispatch(Actions.initSearchRequest());
    ...
    //         dispatch({
    //             type : "setNavigation",
    //             value: AppUtils.getFlatNavigation(navigation).filter(item => itemAuthAllowed(item))
    //         });
    //     }
    //
    //     setNavigation();
...
FuseSearch.defaultProps = {
    trigger: (<IconButton style={{padding:'8px',background:'white', color:'#000'}}><Icon>search</Icon></IconButton>),
    variant: 'normal-search'// basic, full
};

export default withRouter( withReducer('search', searchReducer )(React.memo(FuseSearch)) );
